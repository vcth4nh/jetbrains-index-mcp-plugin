...
### Fixed
- `ide_call_hierarchy` / `ide_type_hierarchy` no longer break the IDE's interactive **Hierarchy tool window** in remote-dev / `serverMode` (JetBrains Gateway). `HierarchyTreeWalker` constructs the platform's `HierarchyBrowserBaseEx` headlessly to read the tree, and that ctor force-initializes the `HierarchyBrowserManager` project service — registered `client="all"` (one instance **per client session**) and whose ctor registers the global "Hierarchy" tool window. Running on a background thread with no `ClientId`, our call resolved to the **local** session and created a *second* instance, colliding with the GUI's controller-session instance: `Conflicting component name 'HierarchyBrowserManager'` + `window with id="Hierarchy" is already registered`, after which `getInstance()` returned null and the interactive Hierarchy action NPE'd on update — so the tool window stopped appearing. The walk now runs under the connected GUI client session (`ClientId.withExplicitClientId` over the project's non-local session, falling back to `localId`) so it shares the GUI's single `HierarchyBrowserManager` instead of spawning a second one. No-op on a plain local IDE (single session). Closes #17.
- `ide_call_hierarchy` / `ide_type_hierarchy` no longer return a spurious `"Browser refused element …"` / failed-hierarchy error when a write action goes pending mid-read. `HierarchyTreeWalker` runs inside the coroutine `readAction { }`, whose read loop automatically restarts the block when a `ReadAction.CannotReadException` (a `ProcessCanceledException`) escapes it. The walker's broad `runCatching {}` blocks were swallowing that cancellation — reflection wraps it in `InvocationTargetException` — so the platform never saw it, never retried, and the null result was mislabeled as a genuine refusal. The walker now rethrows read-action cancellation (unwrapping reflection's `InvocationTargetException`) at every tree-build site so the write-pending restart fires; non-cancellation failures are unaffected. Closes #141.
- `ide_call_hierarchy` `scope` was silently ignored in Go. Language hierarchy browsers that override `getCurrentScopeType()` to read their **own** `myScope` field (e.g. GoLand's `GoCallHierarchyBrowser`) never saw our headless scope poke, which only set the platform sheet's `myScope` — so Go kept the persisted scope and never filtered by `production` / `test` / `this_class`. `HierarchyTreeWalker.applyBrowserScope` now also sets the browser's own `myScope` field when present. No-op for base browsers (Java/Kotlin/Python/PHP/JS/TS), which already honored scope. (RustRover call-hierarchy and PyCharm type-hierarchy don't implement scope filtering at all, so they remain faithful no-ops.)
...
- `ide_find_class` `matchMode` schema enum now declares `camelCase` (an accepted, documented value the tool always handled but never exposed in the enum — surfaced when strict argument validation began enforcing the schema).
...
- **Unified output-format overhaul across all 8 navigation tools** — `ide_find_usages`, `ide_find_definition`, `ide_find_class`, `ide_find_symbol`, `ide_find_implementations`, `ide_find_super_methods`, `ide_type_hierarchy`, `ide_call_hierarchy`. Per-item wire fields are now consistent across every tool. Clients parsing these responses must update. Closes #15, #13, #10, #9.
  ...
  - **Added:** `kind` on `CallElement` (call_hierarchy); `line`/`column` on `TypeElement` (type_hierarchy); `enclosingScope` fallback — present only when `qualifiedName` is null — on find_definition, type_hierarchy, call_hierarchy.
  - **Unified per-item contract:** `name`, `qualifiedName?`, `enclosingScope?`, `kind`, `file`, `line`, `column`, plus tool-specific `preview` / `usageType` / `supertypes` / `children`. Field order is identity → location → recursion.
- **`ide_type_hierarchy` / `ide_call_hierarchy` column now points at the name identifier**, matching `ide_find_class` / `ide_find_symbol` and the IDE's "Go to Declaration" — previously the declaration-start column (the `class` / `def` / `func` keyword). Resolved via the name identifier's `textRange` (new `PsiUtils.identifierOffset`), which works uniformly for source PSI and decompiled `.class` stubs.
...
### Added
- **`ide_install_plugin` tool** (universal, disabled by default). Installs a locally built plugin distribution (`.zip`) into this IDE's custom plugins directory (`PathManager.getPluginsDir()`), replacing any existing copy. Defaults the source to the newest `*.zip` in `<project>/build/distributions/`; accepts an explicit absolute/relative `path`. Unpack is a plain JDK unzip with zip-slip guarding (no dependency on the platform's internal `PluginInstaller`), so it is headless and stable across IDE versions. Reports detected `pluginId`/`pluginVersion` best-effort and always sets `restartRequired: true`. Half of the local plugin dev loop.
- **`ide_restart` tool** (universal, disabled by default). Restarts this IDE via `ApplicationManagerEx.getApplicationEx().restart(true)`. The restart is scheduled *after* the tool's response is flushed (configurable `delaySeconds`, default 2, range 0–60) because the plugin's own MCP server dies with the IDE — the client must receive the result before the connection drops. In remote-dev / `serverMode` the backend relaunches and the thin client reconnects. The other half of the dev loop: `ide_install_plugin` stages the build, `ide_restart` loads it (the plugin is not dynamically reloadable).
...
- **`ide_file_structure` `show` parameter** for selecting which categories (filters / node providers / groupers) are visible. Drives the IDE's structure-view machinery via `TreeModelWrapper` + `SmartTreeStructure` so language-registered toggles work unchanged. Per-language filters available (* = on by default):
  ...
  Defaults are tuned for agent code-reading (e.g. `non_public` and `private_members` are on so agents see implementation details, not just public APIs) — they may differ from the IDE's UI defaults. Omit `show` for defaults, pass `show: ["..."]` to make exactly those categories visible, or `show: []` to hide every filterable category (minimum content). Filter list verified against each language plugin's source (`PublicElementsFilter`/`FieldsFilter`/`JavaAnonymousClassesNodeProvider`/`JavaLambdaNodeProvider` for Java; `PropertiesFilter`/`PublicElementsFilter` for Kotlin; `RsMacroExpandedFilter` for Rust; etc.).
- **`ide_file_structure` `sort` parameter** (single value: `"visibility"`) activates the language-appropriate visibility sorter. Maps to `VISIBILITY_SORTER` for Java/PHP, `KOTLIN_VISIBILITY_SORTER` for Kotlin, `EXPORTABILITY_SORTER` for Go (Go's flavor of "visibility"), and `STRUCTURE_VIEW_VISIBILITY_SORTER` for Rust. No-op for Python and JS/TS, which don't ship a visibility sorter. Default: declaration order with whatever hidden sorters the language model applies silently (kind-grouping for Java/JS/PHP, natural order for Go/PHP).

### Fixed
- **`ide_type_hierarchy` (Java)**: enum-constant subtype rows (e.g. anonymous-class bodies on `enum CoerceMode { INT { … }, ABS { … } }`) no longer surface as truncated raw source like `{ int apply(String s) { return Integer.parseInt(s); } }`. `PsiEnumConstantInitializer` and `PsiAnonymousClass` instances expose `getName() == null`, so wire conversion previously fell through to `psi.text.take(60)`. Now delegates to `ClassPresentationUtil.getNameForClass` (reflective, Java-plugin-gated) which emits `"Enum constant 'INT' in 'demo.Quirks.CoerceMode'"` and `"Anonymous in `enclosingFn()`"` — matching the IDE's Type Hierarchy panel and the existing `ide_find_implementations` formatting.
- **`ide_call_hierarchy` qualifiedName (Java)**: overloaded methods now produce distinct `qualifiedName` values again — e.g. `demo.Quirks#parse(java.lang.String)` and `demo.Quirks#parse(java.lang.String, int)` instead of both collapsing to `demo.Quirks#parse`. The walker's `describeMethodQualifiedName` now defers to the IDE's `JavaQualifiedNameProvider` (via `QualifiedNameUtil`) for `PsiMethod` instances, restoring the parameter signature. Non-Java languages keep the `qualifiedClassName#methodName` synthesis they had before.
- **`ide_call_hierarchy` name (Kotlin)**: top-level functions in call hierarchy now include the JVM facade class prefix again — e.g. `demo.NormalKt.makeDefaultShapes()` instead of bare `makeDefaultShapes()`. K2's call-hier descriptor wraps the `KtNamedFunction` directly with no PSI class ancestor, so `walkUpToClassLike` returned null. The walker now splits the IDE's `QualifiedNameProvider` output at `#` to recover the facade prefix (`demo.NormalKt`) for the wire-format `name` field.
- **`ide_type_hierarchy` kind (Kotlin)**: `abstract class` now reports `kind: "ABSTRACT_CLASS"` instead of `"CLASS"`. `KtClass` exposes no `isAbstract()` accessor (unlike `isData()`/`isSealed()`); the classifier now probes `getModifierList().text` for the `abstract` keyword. Sealed and object classifications are unaffected.
- **`ide_call_hierarchy` callers (JavaScript / TypeScript)**: default scope `project_files` now correctly maps to `SCOPE_ALL` ("All") instead of `SCOPE_PROJECT` ("Production"). Most language plugins implement `SCOPE_PROJECT` as `GlobalSearchScopesCore.projectProductionScope(project)`, which returns an **empty** scope in projects without configured production source roots — a common shape for JS/TS projects without Maven/Gradle setup. `JSCallerMethodsTreeStructure` then found zero callers even when references existed. The new default matches the IDE's own behaviour (its menu invocation pre-selects the "All" tab). `project_production_files` remains mapped to `SCOPE_PROJECT` for users who explicitly want stricter scoping. Side-effect: hierarchy results may include callers from library sources when the target itself is a library symbol; project-internal targets are unaffected.
- **`ide_type_hierarchy` (Rust)**: `BaseRustHandler.resolveReference` now navigates through the embedded `RsPath` when a wrapper element (`RsTraitRef`, `RsTypeReference`) doesn't expose `getReference()` directly. Without this, `resolveReference(traitRef)` returned null for every `impl Trait for Type { … }` construct, which v5.3.4's drop-on-null behavior surfaced as empty `supertypes`/`subtypes`. Resolution chain is now: `element.getReference().resolve()` → `element.resolve()` → `element.getPath().getReference().resolve()`.
- **`ide_type_hierarchy` (Go)**: `kind` now reports `STRUCT` / `INTERFACE` (and `TYPE` for aliases / function types / arrays / maps / channels / pointers) instead of collapsing every Go type to `TYPE`. Root cause: `GoTypeSpec` does not expose `getType()` directly — the IDE chains `getSpecType().getType()` and checks the result against the `GoStructType` / `GoInterfaceType` marker interfaces. `ClassLikePsi.goKind` now uses the same two-hop chain plus `instanceof`-style class loading, surviving any future `Impl`-class renames in the Go plugin.
- **`ide_type_hierarchy` (Rust)**: trait supertypes for structs/enums (and the supertype/subtype branches of an `impl` block hierarchy) no longer surface as `kind: "IMPL"` with a raw `impl Trait for Type { … }` body-source `name`. Root cause: when trait reference resolution failed, the Rust fallback fell back to `psi = impl` (the unnamed `RsImplItem` block); downstream wire conversion grabbed `psi.text` and emitted truncated source. Now drops entries where resolution can't yield a named PSI element, so every supertype/subtype wraps a real `RsTraitItem` / `RsStructItem` / `RsEnumItem` and produces a clean `name="Drawable"`, `kind="TRAIT"` row.
- **`ide_call_hierarchy` (Rust)**: caller results are now uniform with Java / Kotlin / Python / PHP / Go — the column points at the caller method's declaration line, not the call-site reference inside its body. Root cause: `RsCallHierarchyBrowser.getElementFromDescriptor` returns `descriptor.psiElement` (call-site) where Java's returns `descriptor.getEnclosingElement()` (calling method); each plugin made a different choice for the same contract. The walker's `BrowserBackedResolver` now reflectively prefers `descriptor.getEnclosingElement()` when the descriptor exposes it (Rust's `RsCallHierarchyNodeDescriptor` does, returning `RsQualifiedNamedElement`), falling back to the browser's choice otherwise. Picks the IDE's own "logical owner" notion instead of re-implementing walk-up in the wire layer.

- **`ide_file_structure` `show` parameter for JavaScript/TypeScript dialects.** JS files in WebStorm/IntelliJ Ultimate carry PSI language ids `ECMAScript 6` or `JSX Harmony` (and TS `.tsx` files carry `TypeScript JSX`); the previous lookup keyed off `psiFile.language.id.lowercase()` and missed those variants entirely, so `show` was a no-op for the entire JS family. Fixed by routing the language id through `displayLanguageName` first — same canonicalization the response's `language` field already uses — so all JS/TS dialects map to the single `javascript` / `typescript` bucket in `MATCHERS` and `DEFAULTS`.
- **`ide_file_structure` `show` parameter polarity for hide-style filters.** The initial implementation routed every `TreeAction` through `TreeModelWrapper` with `isActionActive == (name in show)`. For `Filter` instances with `isReverted() == true` (the standard "Show X" toggle convention — e.g. `PyInheritedMembersFilter`, `PublicElementsFilter`, `PyFieldsFilter`, `KotlinPublicElementsFilter`, `KotlinPropertiesFilter`, `GoPrivateMembersFilter`), an active filter HIDES its category — so `show: ["inherited"]` was hiding inherited members instead of showing them, and `show: []` produced *maximum* content instead of minimum. NodeProviders and groupers (which add or transform nodes when active) keep the direct mapping — only hide-style filters get the inversion. Fixes Python's default output flooding with inherited dunders, and makes Java/Kotlin defaults actually surface non-public members and properties.
...
- **`ide_find_usages` no longer narrows results when the cursor is on a constructor call or rebinding site.** P0-1 (the `GotoDeclarationHandler`-first resolution chain in `PsiUtils.resolveTargetElement`) was correctly redirecting `new Foo()` cursors to the constructor / `__init__` / class declaration for `find_definition` — but the same redirect was being applied to `find_usages` through the shared resolution chain, narrowing the search anchor and dropping legitimate references (type annotations, variable assignments, transitive uses). `FindUsagesTool` now uses a new `PsiUtils.resolveReferenceTarget` that skips the `GotoDeclarationHandler` step, mirroring the IDE's Find Usages action (Alt+F7) which uses `TargetElementUtil.FIND_TARGET_FLAGS` — those flags do not invoke `GotoDeclarationHandler` extensions. `find_definition` still uses Ctrl+Click semantics. Affects all languages.
...
- **`ide_find_class` and `ide_type_hierarchy` (PHP)**: PHP enums (`enum Foo: string { ... }`) now report `kind: "ENUM"` instead of `"CLASS"`. Both `LanguageAwareKindResolver.resolvePhpKind` and `BasePhpHandler.determineClassKind` now check `PhpClass.isEnum()` reflectively, slotting the branch as `interface > trait > enum > abstract > class`. Backed and pure enums share the `ENUM` kind.
- **`ide_find_class` and `ide_type_hierarchy` (Go)**: Go `struct` and `interface` declarations now report `kind: "STRUCT"` / `"INTERFACE"` instead of `"CLASS"`. The PSI tree is `GoTypeSpec → GoSpecType → GoStructType | GoInterfaceType`; the previous reflective `getSpecType().isInstance(GoStructType)` check always failed because the wrapper `GoSpecType` is not itself a struct/interface type. Both kind-classification sites now use `PsiTreeUtil.findChildOfType` to walk past the wrapper.
- **`ide_type_hierarchy` (JavaScript)**: now correctly reports direct subtypes only. The previous strategy 1 reflectively imported a non-existent `JSInheritorsSearch` class — every `Class.forName` threw `ClassNotFoundException`, the outer catch swallowed it, and execution silently fell through to strategy 2 (`DefinitionsScopedSearch`), which returns transitive subtypes via `JSClassSearch.searchClassInheritors(_, checkDeep=true)`. Strategy 1 now uses the canonical `JSInheritanceUtil.findDirectSubClasses` API (same as IntelliJ's `JSSubtypesHierarchyTreeStructure.buildChildren`); strategy 2 wraps each result through `JSInheritanceUtil.isParentClass(_, _, false)` for defense.
- **`ide_file_structure` (Python)**: nested functions and classes inside function bodies are now surfaced as children of the enclosing function (was: silently dropped). `extractFunctionStructure` now mirrors `PyStructureViewElement.getElementChildren`'s recursive-descent visitor — descends through non-worthy container nodes (`PyStatementList`, `PyIfPart`, `PyWithStatement`) and stops at worthy ones (`PyFunction`, `PyClass`). Conditional defs (`if x: def foo(): ...`) now also surface, matching PyCharm's Structure View.

### Changed
- **`ide_file_structure` now delegates to `LanguageStructureViewBuilder`** — the IDE's own structure-view dispatcher. The five hand-written per-language handlers (Java, Kotlin, Python, JS/TS, Markdown) are deleted in favor of a single language-agnostic walker over `StructureViewTreeElement`. Net deletion: ~3,000 LOC of plugin code (`JavaStructureHandler`, `KotlinStructureHandler`, `PythonStructureHandler`, `JavaScriptStructureHandler`, `TypeScriptStructureHandler`, `MarkdownStructureHandler`, the `StructureHandler` interface, and the `StructureKind` enum).

  **Wire format**: the JSON shape (`{file, language, structure}`) is unchanged. The `structure` string content is now rendered from the IDE's own `ItemPresentation.presentableText` for each tree node (e.g. `myMethod(String, int): boolean`, `Circle(Shape)`), matching what IntelliJ shows in its Structure tool window. The previous `kind`/`modifiers` prefixes (e.g. `class `, `def `, `private static`) are gone — that information is now folded into the IDE's presentable text. Anyone parsing the formatted string for those prefixes will need to adjust.
...
- **`ide_find_definition`** (and any tool using `PsiUtils.resolveTargetElement` — `ide_find_usages`, `ide_find_implementations`, `ide_find_super_methods`, `ide_call_hierarchy`): now tries `GotoDeclarationHandler.EP_NAME.extensionList` first before falling back to `PsiReference.resolve()`. This mirrors what the IDE's Ctrl+Click does (`GotoDeclarationAction.findTargetElementsNoVS`) — language-registered handlers can express navigation rules (e.g., constructor calls → class) that pure reference resolution misses. Existing parent-walk + named-element fallbacks unchanged. Python skeleton↔typeshed refinement still applies to every step.
- **`ide_find_class`**: now drives IntelliJ's Go to Class popup stack via a new `PopupFaithfulClassSearch` (mirror of `PopupFaithfulSymbolSearch` but using `GotoClassModel2`). Inherits the IDE's parallel execution, dumb-mode safety, cancellation, proximity-aware sorting, and qualified-query support. Replaces the manual `CLASS_EP_NAME.extensionList` iteration. `matchMode` (`substring` / `prefix` / `exact`) now applies as a post-filter using `SearchMatchUtils` — substring is the popup's native default; the other modes narrow it. No user-visible behavior change for the common case.

### Removed
- `StructureHandler` interface, its per-language implementations (`JavaStructureHandler`, `KotlinStructureHandler`, `PythonStructureHandler`, `JavaScriptStructureHandler`, `TypeScriptStructureHandler`, `MarkdownStructureHandler`), the `MarkdownHandlers` registration object (P0-4), the `StructureKind` enum, the `modifiers` and `kind` fields on `StructureNode`, and the `StructureKind`-keyed switch in `TreeFormatter`. `LanguageHandlerRegistry` no longer carries `registerStructureHandler` / `getStructureHandler` / `hasStructureHandlers` / `getSupportedLanguagesForStructure` methods.
